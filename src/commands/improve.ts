...
// Benchmarks are now defined in dataFormatter.ts as BENCHMARK_DECIMALS
// This provides consistent decimal format (0.0-1.0) for all internal comparisons
...
interface SideSpecificInsights {
  ctInsights: string[];
  tInsights: string[];
  ctDrills: string[];
  tDrills: string[];
...
function analyzeCTTSideWeaknesses(
  ...
  // General side analysis using raw decimal ratings (more meaningful than converted percentages)
  if (ctInsights.length === 0 && ctRatingRaw < 0.05) {
    // Check if individual stats are actually reasonable despite low rating
    if (ctOpeningPct >= 37 && ctAggressionPct >= 35) {
      const ctTrafficLight = ctRatingRaw >= 0.05 ? '🟢 Average' : ctRatingRaw >= 0.0 ? '🟡 Below average' : '🔴 Significant improvement needed';
      ctInsights.push(`CT side performance: ${ctTrafficLight} (${ctRatingRaw.toFixed(3)}) despite reasonable individual stats`);
    ...
    } else if (ctRatingRaw < 0.0) {
      ctInsights.push(`CT side performance: 🔴 Significant improvement needed (${ctRatingRaw.toFixed(3)}) - major structural issues`);
      ...
      ctInsights.push(`CT side performance: 🟡 Below average (${ctRatingRaw.toFixed(3)}) - fundamental CT positioning needs work`);
  ...
  if (tInsights.length === 0 && tRatingRaw < 0.05) {
    // Check if individual stats are actually good despite low overall rating
    if (tOpeningPct >= 37 && tAggressionPct >= 40) {
      const tTrafficLight = tRatingRaw >= 0.05 ? '🟢 Average' : tRatingRaw >= 0.0 ? '🟡 Below average' : '🔴 Significant improvement needed';
      tInsights.push(`T side performance: ${tTrafficLight} (${tRatingRaw.toFixed(3)}) despite reasonable individual stats`);
    ...
    } else if (tRatingRaw < 0.0) {
      tInsights.push(`T side performance: 🔴 Significant improvement needed (${tRatingRaw.toFixed(3)}) - major fundamental issues`);
      ...
      tInsights.push(`T side performance: 🟡 Below average (${tRatingRaw.toFixed(3)}) - entry fundamentals need work`);
      tInsights.push(`→ Stats: T opening duels ${tOpeningPct.toFixed(1)}%, aggression success ${tAggressionPct.toFixed(1)}%`);
...
function analysePositioning(stats: RawLeetifyProfile['stats'], rating: number, weakSide?: 'CT' | 'T' | null): ImprovementArea {
  const issues: string[] = [];
  const drills: string[] = [];
  const resourceTags: string[] = [];
  ...
  if (issues.length === 0) {
    ...
    return {
      ...
      rating,
      emoji: '🗺️',
      issues: [], // Empty issues means this won't be selected as focus area
      drills: [],
      resourceTags: []
    };
  }
...
function analyseOpening(stats: RawLeetifyProfile['stats'], rating: number, weakSide?: 'CT' | 'T' | null): ImprovementArea {
  const issues: string[] = [];
  const drills: string[] = [];
  const resourceTags: string[] = [];
  ...
  if (!ctSuccessLow && !tSuccessLow && rating < BENCHMARK_DECIMALS.ratings.opening && rating >= 0.20) {
    issues.push('Opening rating is low but individual duel success is ok — you may be taking too few opening duels');
    ...
    issues.push('Opening rating is extremely low — this may indicate you rarely take opening duels');
...
function analyseClutch(stats: RawLeetifyProfile['stats'], rating: number): ImprovementArea {
  ...
  if (rating < 15) {
    issues.push(`Clutch rating is extremely low (${rating.toFixed(0)}/100) — this suggests very few clutch attempts`);
    ...
    issues.push(`Clutch rating is very low (${rating.toFixed(0)}/100) — losing most 1vX situations badly`);
  ...
  } else if (rating < (BENCHMARK_DECIMALS.ratings.clutch * 100)) {
    issues.push(`Clutch performance could improve (${rating.toFixed(0)}/100) but this is normal for most players`);
...
function analyseSideBalance(
  ...
  if (weakSide === 'CT') {
    ...
    return {
      ...
      weakSide: 'CT',
      advice: `Your CT side is weaker than T side (${weakRating.toFixed(3)} vs ${strongRating.toFixed(3)}). ` +
    ...
    return {
      hasSideImbalance: true,
      weakSide: 'T',
      advice: `Your T side is weaker than CT side (${weakRating.toFixed(3)} vs ${strongRating.toFixed(3)}). ` +
...
function detectCrossInsights(
  ...
  if (ratings.aim >= 55 && ratings.opening < 40) {
    insights.push({
  ...
  if (ratings.aim >= 55 && ratings.clutch < 35) {
    insights.push({
  ...
  if (ratings.utility < 40 && ratings.opening >= 55) {
    insights.push({
...
function buildImprovementReport(rawProfile: RawLeetifyProfile): {
  ...
  // convert leetify ratings to 0-100 — some come as 0-1, some already 0-100
  const to100 = (v: number) => Math.round(v > 1 ? v : v * 100);
  const ratings = {
    aim: to100(rating.aim || 0),
    positioning: to100(rating.positioning || 0),
    utility: to100(rating.utility || 0),
    clutch: to100(rating.clutch || 0),
    opening: to100(rating.opening || 0),
  };

  const ctRating = to100(rating.ct_leetify || 0);
  const tRating = to100(rating.t_leetify || 0);
  ...
  // Step 1: Always include critically low areas (< 30) regardless of category
...
function buildImprovementEmbed(
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Based on your last 30 games. **${focusAreas.length} key area${focusAreas.length !== 1 ? 's' : ''}** need attention:`
    );

  // rating overview — all 5 categories at a glance + CT/T ratings
  const ratingBar = areas.map(a => {
    const bar = a.rating >= 60 ? '🟢' : a.rating >= 45 ? '🟡' : '🔴';
    const ratingDisplay = `${Math.round(a.rating)}/100`;
    return `${bar} **${a.category}**: ${ratingDisplay}`;
  }).join('\n');
  ...
  // Convert raw leetify ratings to traffic light descriptions
  function getTrafficLightRating(rating: number): { emoji: string; description: string } {
    if (rating >= 0.1) return { emoji: '🟢', description: 'Above average performance' };
    if (rating >= 0.05) return { emoji: '🟢', description: 'Average performance' };
    if (rating >= 0.0) return { emoji: '🟡', description: 'Below average - room for improvement' };
    return { emoji: '🔴', description: 'Significant improvement needed' };
  }
    
  const sideRatings = [];
  if (ctLeetifyRaw !== 0) {
    const ctRating = getTrafficLightRating(ctLeetifyRaw);
    sideRatings.push(`${ctRating.emoji} **CT Side**: ${ctRating.description} (${ctLeetifyRaw.toFixed(3)})`);
  }
  if (tLeetifyRaw !== 0) {
    const tRating = getTrafficLightRating(tLeetifyRaw);
    sideRatings.push(`${tRating.emoji} **T Side**: ${tRating.description} (${tLeetifyRaw.toFixed(3)})`);
  }
  ...
  for (const area of focusAreas) {
    const ratingDisplay = `${Math.round(area.rating)}/100`;
    const isPriority = area.rating < 30 ? '🚨' : area.rating < 45 ? '⚠️' : '📈';
  ...
  const goodAreas = areas
    ...
    .map(a => `${a.emoji} ${a.category} (${Math.round(a.rating * 10) / 10}/100)`)
    .join(' • ');
